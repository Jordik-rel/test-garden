...
@section('title')
    <title>Valeur Nutritionnnelle - Biblitoheque</title>
@endsection
    ...
    <div class="px-4 sm:px-6 lg:px-8">
        <div class="sm:flex sm:items-center">
            <div class="sm:flex-auto">
                <h1 class="text-xl font-semibold text-gray-900">Valeurs Nutritionnelles</h1>
                <p class="mt-2 text-sm text-gray-700">
                    La liste des différentes valeurs nutritionnelles des plantes avec leur nom, date d'ajout.
                </p>
            </div>
                                ...
                                <div class="modal-body">
                                    <div class="bg-white px-4 py-2 rounded-md">
                                        <h3 class="text-center text-lg">
                                            Ajouter une valeur nutrtionnelle
                                        </h3>
                                        <form action="{{ route('admin.valeur.store') }}" method="POST" class="mt-6 flex flex-col justify-between">
                                            @csrf
                                            <div class="mb-3">
                ...
                <div class="inline-block min-w-full py-2 align-middle sm:px-6 lg:px-8">
                    <table class="min-w-full divide-y divide-gray-300">
                        ...
                        <tbody class="divide-y divide-gray-200 bg-white">
                            ...
                            @forelse ($valeurNutritionnelles as $valeurNutritionnelle)
                                <tr>
                                    <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-0">
                                        {{ $valeurNutritionnelle->nom }}
                                    </td>
                                    <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{{ $valeurNutritionnelle->created_at->format('d M, Y') }}</td>
                                    <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{{ $valeurNutritionnelle->plant->count() }}</td>
                                    ...
                                    <td class="flex justify-center items-center whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                        ...
                                        <a href="#" 
                                            data-bs-toggle="modal" 
                                            data-bs-target="#modifierModal{{ $valeurNutritionnelle->id }}"
                                        ...
                                        <!-- Delete -->
                                        <form action="{{route('admin.valeur.destroy',$valeurNutritionnelle)}}" method="post">
                                            @csrf
                                            @method('DELETE')
                                        ...
                                        <!-- Modal UPDATE -->
                                        <div class="modal fade" id="modifierModal{{ $valeurNutritionnelle->id }}" tabindex="-1" aria-hidden="true">
                                            <div class="modal-dialog modal-dialog-centered">
                                                <div class="modal-content">
                                                    ...
                                                    <div class="relative bg-white rounded-xl shadow-xl px-8 py-6 w-[500px] mx-auto">
                                                        ...
                                                        <h3 class="text-center text-sm mb-4">
                                                            Modifier {{ $valeurNutritionnelle->nom }}
                                                        </h3>

                                                        <!-- FORMULAIRE -->
                                                        <form action="{{ route('admin.valeur.update', $valeurNutritionnelle) }}" method="POST" class="space-y-4">
                                                            @csrf
                                                            @method('PUT')

                                                            <div>
                                                                <label for="nom-{{ $valeurNutritionnelle->id }}" class="form-label text-black font-semibold px-1 mb-1">
                                                                    Nom <span class="text-red-600">*</span>
                                                                </label>
                                                                <input type="text" 
                                                                    name="nom" 
                                                                    id="nom-{{ $valeurNutritionnelle->id }}"
                                                                    value="{{ old('nom', $valeurNutritionnelle->nom) }}"
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="text-center py-4 text-gray-500">Aucune valeur nutritionnelle trouvée.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                     {{ $valeurNutritionnelles->links() }}
                </div>
            </div>
        </div>
    </div>
