...
<body class="bg-gray-100">
    ...
    <aside id="sidebar"
        ...
        <nav class="px-4 py-4 space-y-12 text-sm flex flex-col justify-between">
            <div class="space-y-2">

                <a href="{{ route('jardinier.recap.dashboard') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-gray-100">
                    <i class="bi bi-speedometer2"></i> Dashboard
                </a>
    
                <a href="{{ route('jardinier.recap.soumission') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-gray-100">
                    <i class="bi bi-ui-checks"></i> Mes soumissions
                </a>
    
                <a href="{{ route('jardinier.recap.projets') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-gray-100">
                    <i class="bi bi-bar-chart"></i> Mes projets
                </a>
    
                <a href="{{ route('jardinier.myaccount') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-gray-100">
            ...
            <form action="{{ route('logout') }}" method="POST">
                @csrf
                <button type="submit" class="w-full text-red-600 text-left flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-red-100">
        ...
        <header class="bg-white h-16 border-b border-gray-200 flex items-center justify-between px-6">
            ...
            <div class="flex items-center gap-6">
                ...
                <div class="flex items-center gap-2 cursor-pointer">
                    <img src="{{ $user->profile_photo_path ? asset('storage/'.$user->profile_photo_path) : asset('https://i.pravatar.cc/40?img=12') }}" class="w-10 h-10 rounded-full" />
