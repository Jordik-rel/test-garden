...
@section('content')
    <div class="p-6 bg-white rounded-xl shadow-sm min-h-full flex md:grid-cols-2 justify-between">
        <div class="w-full {{ $mission->projet->status == 3 ? 'md:w-full' : 'md:w-[70%]' }}  mx-auto py-10 px-6">
            ...
            <div class="bg-white border rounded-xl shadow-sm overflow-hidden">
                ...
                <div class="p-6 border-b flex justify-between">
                    <h2 class="text-xl font-semibold mb-2">
                        {{ $mission->projet->titre ?? 'Titre non défini' }}
                    </h2>
                </div>
                ...
                <div class="p-6 border-b flex flex-col justify-between">
                    ...
                    <p class="whitespace-pre-line text-gray-700">
                        {{ $mission->projet->description ?? 'Aucune description fournie' }}
                    </p>
                </div>
                ...
                <div class="p-6 border-b flex flex-col justify-between">
                    <div>
                        ...
                        @forelse ($mission->projet->competence as $competence)
                ...
                <div class="p-6 border-b flex flex-col justify-between">
                    ...
                    <ul class="text-gray-700 font-medium w-full">
                        <li class="flex justify-between items-center w-full space-y-3">
                            ...
                            @switch($mission->projet->taille_poste)
                                @case(1)
                                    Petit
                                    @break
                                @case(2)
                        ...
                        <li class="flex justify-between items-center w-full space-y-3">
                            ...
                            @switch($mission->projet->niveau_experience)
                                @case(1)
                                    Débutant
                                    @break
                                @case(2)
                        ...
                        <li class="flex justify-between items-center w-full space-y-3">
                            ...
                            @switch($mission->projet->type_emploi)
                                @case(0)
                                    Projet ponctuel
                                    @break
                                @case(1)
                ...
                <div class="p-6 border-b flex justify-between">
                    <div>
                        ...
                        <p class="text-gray-700">
                            @if($mission->projet->tarif_type == 0)
                                De {{ $mission->projet->tarif_min }} Franc CFA/h à {{ $mission->projet->tarif_max}} Franc CFA/h
                            @elseif($mission->projet->tarif_type == 1)
                                {{ $mission->projet->budget}} Franc CFA (prix fixe)
                            @else
                                Non défini
                            @endif
                        </p>
                ...
                <div x-data="{ showModal: false }">
                    ...
                    >
                        ...
                        >
                            ...
                            Etes vous sur d'approuver la fin de votre mission? 
                            </p>

                            <p class="text-red-600 mt-1 text-sm leading-relaxed">Etes-vous sûr d'avoir finis le travail pour lequl avous avez été recruté ?.</p>
                            ...
                            <div class="flex justify-between mt-6 gap-4">
                                ...
                                <form action="{{ route('jardinier.projets.validation',$mission) }}" method="POST" class="w-1/2">
                                    @csrf
                                    @method('PUT')
